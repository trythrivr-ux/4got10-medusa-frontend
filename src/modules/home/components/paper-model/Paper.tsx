...
interface PaperProps {
  ...
  onSceneReady?: () => void // Callback when all textures are loaded
}
...
export const Paper = ({
  ...
  const totalTexturesRef = useRef(NUM_PAGES)
  ...
  const pages = useMemo(() => {
    return Array.from({ length: NUM_PAGES }, (_, i) => {
      ...
      const isExternalUrl =
        texturePath.startsWith("http://") || texturePath.startsWith("https://")
      ...
      const texture = loader.load(
        ...
        (loadedTexture) => {
          ...
          console.log(
            `Loaded ${loadedTexturesRef.current}/${totalTexturesRef.current} textures`
          )

          // Call onSceneReady when all textures are loaded
          if (
            loadedTexturesRef.current === totalTexturesRef.current &&
            onSceneReady
          ) {
            console.log("All textures loaded, scene ready!")
            onSceneReady()
          }
        },
        undefined,
        (error) => {
          ...
          console.warn(
            `Failed to load texture for page ${i}:`,
            texturePath,
            error
          )
          ...
          // Track loaded textures (placeholders count too)
          loadedTexturesRef.current++
          console.log(
            `Loaded ${loadedTexturesRef.current}/${totalTexturesRef.current} textures (placeholder)`
          )
          ...
          if (
            ...
            console.log(
              "All textures loaded (including placeholders), scene ready!"
            )
            onSceneReady()
          }
        }
  ...
  const curvePath = useMemo(() => {
    return new THREE.CatmullRomCurve3(
      [
        new THREE.Vector3(0, -2.5, 0.5), // A: start from way below viewport (fully out of screen)
        new THREE.Vector3(0, -2.0, 0.4), // B: moving up from far below
        new THREE.Vector3(0.2, -1.5, 0.3), // C: swing right while moving up
        new THREE.Vector3(-0.2, -1.0, 0.2), // D: swing left while moving up
        new THREE.Vector3(0, -0.5, 0.1), // E: approaching center from below
        new THREE.Vector3(0, 0, 0), // F: centered in front of camera (end)
      ],
      false,
      "catmullrom",
      0.5
  ...
  useFrame((state) => {
    ...
    const baseDistance =
      PAGE_HEIGHT * (viewportHeightAtDistance1 / targetViewportHeight)

    // Use curve path Y position for vertical animation (scaled to viewport)
    // The curve Y values range from -1.5 to 0, so we scale them to viewport height
    const scaledY = frame.position.y * targetViewportHeight

    // Position at calculated distance from camera with curve-based Y position
    // Offset X by -PAGE_WIDTH/2 to center the magazine (geometry pivots at spine/left edge)
    groupRef.current.position.set(
      frame.position.x - PAGE_WIDTH / 2,
      scaledY, // Use scaled curve Y position for bottom-to-top animation
      -baseDistance + frame.position.z // Negative Z = toward camera
    )
    ...
    pages.forEach((page, index) => {
      ...
      for (let i = 0; i < PAGE_SEGMENTS; i++) {
        ...
        const leafBend =
          Math.sin(segmentProgress * Math.PI) * 0.18 * leafIntensity
