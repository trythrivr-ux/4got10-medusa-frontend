...
export default function LandingPageClient() {
  ...
  useEffect(() => {
    ...
    // Debug log
    console.log(
      "Effect running, containerRef:",
      containerRef.current,
      "paperSceneRef:",
      paperSceneRef.current
    )
    ...
    const initializeAnimations = () => {
      ...
      console.log("Refs ready, waiting for scene to load...")

      // Set up scene ready callback
      if (paperSceneRef.current.setOnSceneReady) {
        paperSceneRef.current.setOnSceneReady(() => {
          console.log("Scene ready! Setting initial animation state...")

          // Set initial animation state to closed AFTER scene is ready
          if (paperSceneRef.current) {
            paperSceneRef.current.setScrollProgress(0)
            console.log("Set initial animation state: closed")
          }

          // Now initialize the rest of the animations
          setupScrollTriggers()
        })
      } else {
        // Fallback if setOnSceneReady is not available
        console.log("setOnSceneReady not available, using fallback")
        setupScrollTriggers()
      }
    }
    ...
    const setupScrollTriggers = () => {
      ...
      const handleScroll = () => {
        ...
        // Sections 0+1: curve path animation, scrollProgress 0→1 (complete earlier)
        // Section 2: magazine exits out of screen (reverse animation)
        // Section 3: bend animation
        // Section 4: reset + unpin
        if (currentSection <= 1) {
          // Scroll from top of page to bottom of section 1 → progress 0 to 1 (complete earlier)
          const animEnd = sectionHeight * 2 // bottom of section 1
          const progress = currentScrollY / animEnd
          paperSceneRef.current.setScrollProgress(
            Math.max(0, Math.min(1, progress))
          )
          paperSceneRef.current.setBendAmount(0)
        } else if (currentSection === 2) {
          // Magazine exits out of screen (reverse animation to bottom)
          const sectionProgress =
            (currentScrollY - sectionHeight * 2) / sectionHeight
          const exitProgress = 1 - Math.min(1, Math.max(0, sectionProgress)) // 1 to 0
          paperSceneRef.current.setScrollProgress(exitProgress)
          paperSceneRef.current.setBendAmount(0)
        } else if (currentSection === 3) {
          paperSceneRef.current.setScrollProgress(0) // stays at bottom
          const sectionProgress =
            (currentScrollY - sectionHeight * 3) / sectionHeight
          const bendValue =
            Math.sin(Math.min(1, Math.max(0, sectionProgress)) * Math.PI) * 6
      ...
      const updateScenePosition = (scrollY: number, sectionHeight: number) => {
        ...
        const scene3D = containerRef.current.querySelector(
          ".fixed-3d-scene"
        ) as HTMLElement
        const gradientBg = containerRef.current.querySelector(
          ".gradient-background"
        ) as HTMLElement
      ...
      const updateStartButtonVisibility = () => {
        ...
        const startButton = containerRef.current.querySelector(
      ...
      const updateGradientColors = (section: number) => {
        ...
        const gradientOverlay = containerRef.current.querySelector(
          ".gradient-overlay"
        ) as HTMLElement
      ...
      const animateFullSequence = () => {
        ...
        const finalProgress =
          easedProgress * easedProgress * (3.0 - 2.0 * easedProgress)

        paperSceneRef.current.setScrollProgress(finalProgress)

        // Continue animation if not complete
        if (rawProgress < 1) {
          momentumRef.current.animationId =
            requestAnimationFrame(animateFullSequence)
        } else {
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="h-screen w-full bg-transparent flex items-center justify-center"
      >
      ...
      >
        <div className="text-center text-white z-10">
          <h2 className="text-3xl font-bold mb-2 font-[family-name:var(--font-jersey)]">
            Explore
          </h2>
          <p className="text-white/80 font-[family-name:var(--font-kumbh)]">
